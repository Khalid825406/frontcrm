...
//       const res = await axios.get('http://localhost:5000/api/admin/assigned-jobs-status', {
...
//     const token = localStorage.token;
//     await axios.post(`http://localhost:5000/api/admin/accept-job/${jobId}`, {}, {
//       headers: { Authorization: `Bearer ${token}` },
//     });
//     fetchJobs();
//   };
...
//     const token = localStorage.token;
//     await axios.post(`http://localhost:5000/api/admin/reject-job/${selectedJobId}`, {
//       reason,
//     }, {
//       headers: { Authorization: `Bearer ${token}` },
//     });
...
//     const token = localStorage.token;
//     await axios.post(`http://localhost:5000/api/admin/update-status/${jobId}`, {
//       status: newStatus,
//     }, {
//       headers: { Authorization: `Bearer ${token}` },
//     });
...
//               <h4 className="job-title">{job.customerName}</h4>
...
//                 {job.status === 'Accepted' && (
//                   <button onClick={() => updateStatus(job._id, 'In Progress')} className="btn progress-btn">Start Work</button>
//                 )}

//                 {job.status === 'In Progress' && (
//                   <button onClick={() => updateStatus(job._id, 'Completed')} className="btn complete-btn">Mark as Completed</button>
//                 )}
//               </div>
//             </div>
//           ))
...
// export default function TechnicianJobsPage({username}) {
//   const [jobs, setJobs] = useState([]);
//   const [loading, setLoading] = useState(true);
//   const [showRejectModal, setShowRejectModal] = useState(false);
//   const [reason, setReason] = useState('');
...
//                 {job.images?.length > 0 && (
//                   <img
//                     src={`http://localhost:5000${job.images[0]}`}
//                     alt="Job"
//                     className="job-image"
//                   />
//                 )}
...
//       setUser({ username: res.data.user });  // 👈 wrap the string
//     }
//   })
//   .catch(err => {
